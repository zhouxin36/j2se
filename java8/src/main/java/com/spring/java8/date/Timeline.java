...
public class Timeline {
    public static void main(String[] args) {
        ...
        System.out.println(timeElapsed.toMillis() + " milliseconds\n");

        Instant start2 = Instant.now();
        runAlgorithm2();
        Instant end2 = Instant.now();
        Duration timeElapsed2 = Duration.between(start2, end2);
        System.out.println(timeElapsed2.toMillis() + "milliseconds\n");

        boolean overTenTimesFaster = timeElapsed.multipliedBy(10).minus(timeElapsed2).isNegative();
        System.out.println("The first algorithm is " + (overTenTimesFaster ? "" : "not ") + "than ten times faster");
    }

    private static void runAlgorithm() {
        int size = 10;
        List<Integer> list = new Random().ints().map(i -> i % 100).limit(size).boxed().sorted().collect(Collectors.toList());
